...
| `sizes` | `Size` | name, **group**, coefficient |
| `work_item_types` | `WorkItemType` | name, **group**, coefficient, slug |
...
### 作業種別（work_item_types）のグループ分類

`group` カラム（Migration: `2026_03_28_000001_add_group_to_work_item_types_and_sizes.php`）で分類。

| group 値 | 日本語名 | 主な担当 | 代表的な種別 |
|---|---|---|---|
| `dtp` | 組版・DTP | 組版オペレーター | 組版（新規作成）[creation]、組版（修正・赤字）[revision]、赤字照合、データ変換・整備 |
| `design` | デザイン・制作 | デザイナー | デザイン（新規）[design_new]、デザイン（修正）[design_revision]、画像処理・レタッチ[image_processing]、原稿整理・編集[editing] |
| `proof` | 校正・確認 | 校正者 | 校正[proofreading]、確認・照合[confirmation] |
| `mgmt` | 進行管理・事務 | 進行管理 | 書類作成[document_creation]、データ整理・管理[data_management]、打合せ・確認対応[meeting] |
| `sales` | 営業 | 営業 | 架電・電話対応[sales_call]、顧客訪問・営業活動[sales_visit]、見積・提案書作成[sales_estimate]、受注・発注処理[sales_order]、伝票・請求書処理[sales_invoice]、社内報告・情報共有[sales_report] |
| `common` | 共通 | 全担当 | その他[other]（slug='other' は変更禁止） |

**AssignmentForm の業種フィルター:** `block._type_filter` に group 値をセット → `typesGrouped()` が `<optgroup>` で分類して表示。
**割当ユーザー選択時の自動フィルター:** `onUserChange(block)` が `assignment_name` → group 値にマッピングして `_type_filter` を自動設定。

マッピング（`ASSIGNMENT_TYPE_MAP`）:
- 「組版」「オペレーター」「DTP」→ `dtp`
- 「デザイナー」「デザイン」→ `design`
- 「校正」→ `proof`
- 「進行管理」→ `mgmt`
...
| group 値 | 日本語名 | 代表的なサイズ |
|---|---|---|
| `paper` | 紙媒体 | A3超、A2、A3、A4、A5、B4、B5、B6、四六判、タブロイド、変形（小）、変形（大） |
| `digital` | デジタル・Web | Web（PC）、Web（タブレット）、Web（モバイル）、ファイル単位 |

**AssignmentForm の媒体フィルター:** `block._medium_filter` に `'paper'` or `'digital'` をセット（デフォルト: `'paper'`）。

**コントローラーでの取得:** `types` / `sizes` を取得する際は必ず `group` カラムを含めること。
```php
// 正しい書き方（group を含める）
$types = WorkItemType::orderBy('sort_order')->get(['id', 'name', 'group', 'company_id', 'department_id']);
$sizes = Size::orderBy('sort_order')->get(['id', 'name', 'group', 'width', 'height', 'unit', 'company_id', 'department_id']);
```
...
#### ② 職種グループ別パーセンタイルランク変換（最重要）

担当（職種）によって作業量の規模が根本的に異なるため（例：校正者は月に数百ページ処理するが組版オペレーターは数十ページ）、
**同じ担当グループ内でパーセンタイル（0〜100）に変換**してから合算する。

```
比較対象の決定ルール:
  1. 同部署の全メンバーを assignment_name（担当名）でグループ化
  2. グループ人数 N ≥ 3 → 同職種グループ内でパーセンタイルを計算（"role" レベル）
  3. グループ人数 N < 3 → 部署全体を比較対象にフォールバック（"department" レベル）

パーセンタイルの計算式:
  above = 自分より生スコアが高いメンバー数（同グループ内）
  tied  = 自分と同じ生スコアのメンバー数（同グループ内）
  avgRank = above + (tied + 1) / 2      ← 同値タイは平均順位
  パーセンタイル = (N − avgRank) / (N − 1) × 100
  ※ N = 1 の場合は 100 固定
```

- index() では `$computePct` クロージャで計算。`$roleGroups` に assignment_name でグループ分け後、N≥3 なら role、N<3 なら dept の pct を使用
...
- `member.aggregates.comparison_level` に `'role'` または `'department'` が入る

**show() での扱い:**
- `$viewerAssignment`（対象ユーザーの assignment_name）で同職種 UID リスト `$sameRoleUids` を収集
- N≥3 なら role グループ内で `$viewerPercentile` を計算、N<3 なら会社全体でフォールバック
- `comparison_level`, `peer_group_size`, `peer_assignment` を Inertia props として返す
- Show.vue では「比較グループ」バッジを表示する

**校正者と組版オペレーターの公平な比較の仕組み:**
- 校正者5人がいれば、その5人同士でページ系スコアを比較 → 校正者の中で1位が100点
- 組版オペレーター4人がいれば、その4人同士で比較 → 組版の中で1位が100点
- それぞれの担当の中で「どれだけ頑張ったか」を平等に評価
...
```
比較グループ: 同会社の全ユーザー（全担当混合）
z = (自分の総合ポイント − グループ平均) / グループ標準偏差
偏差値 = 50 + 10 × z
```

偏差値の比較母集団は「会社全体（全担当混合）」。パーセンタイルの母集団「同職種グループまたは部署」とは異なる。
偏差値は異なる担当を横断した大まかな位置把握用の参考値として使用する。

#### ⑤ カテゴリ別順位（show ページ用）

```
比較対象: 職種グループ内（N≥3 の場合）または会社全体（フォールバック）
各カテゴリの生スコアで降順ソート → 1位から順位付与
同値タイ: 全員に same rank（above のカウント方式）
```

show ページの `category_ranks.{cat}` に格納。`group_count`（旧）→ `peer_group_size` が比較人数、`comparison_level` が比較レベル。
...
4. 【職種グループ別パーセンタイル計算】部署単位 × assignment_name グループ単位
   - $computePct クロージャ（UID リスト + スコアマップ → パーセンタイル）
   - 部署内の assignment_name でグループ分け → $roleGroups
   - グループ N ≥ 3: role グループ内パーセンタイル → comparison_level = 'role'
   - グループ N < 3: 部署全体パーセンタイル（フォールバック） → comparison_level = 'department'
   - percentile_scores, points.overall, comparison_level を付与
...
8. 【職種グループ別パーセンタイル計算】
   - $showAssignmentMap: グループ全員の assignment_name を取得
   - $viewerAssignment: 対象ユーザーの assignment_name
   - $sameRoleUids: 同じ assignment_name を持つ UID リスト
   - N ≥ 3: $sameRoleUids 内でパーセンタイル計算 → comparison_level = 'role'
   - N < 3: 会社全体でパーセンタイル計算（フォールバック） → comparison_level = 'department'
   - $viewerPercentile: 各カテゴリのパーセンタイル → percentile_scores
   - $categoryRanks: ピア内順位 → category_ranks
9. Inertia::render で全データを渡す
   - 追加 props: comparison_level, peer_group_size, peer_assignment
```
...
- **比較グループバッジ**: `comparison_level`, `peer_assignment`, `peer_group_size` を表示。「同担当○○内（N人）」または「部署全体（N人）※フォールバック」
- **レーダーチャート**: `percentile_scores` の 6カテゴリ値（0〜100）を使用。生スコアは使わない
- **合計ポイント表示**: `overallPoints = percentile_scores.overall`（0〜600）
- **計算方法モーダル**: `showCalcModal` フラグ。モーダル内に概要・詳細を日本語で記載済み（職種グループ別パーセンタイルの説明を含む）

**新規 props（比較グループ情報）:**
```
comparison_level  : 'role' | 'department'  // 職種グループ比較 or 部署全体フォールバック
peer_group_size   : Number                 // 比較対象人数
peer_assignment   : String                 // 比較対象の担当名（comparison_level='role' の場合）
```
...
# 2. 依存関係インストール（legacy-peer-deps は .npmrc で自動適用）
docker compose exec laravel bash -lc "npm install"

# 3. マイグレーション実行
docker compose exec laravel bash -lc "php artisan migrate"

# 4. Superadmin 作成（.env に SUPERADMIN_EMAIL / SUPERADMIN_PASSWORD を設定後）
docker compose exec laravel bash -lc "php artisan db:seed --class=CreateSuperadminCompanySeeder"
docker compose exec laravel bash -lc "php artisan db:seed --class=CreateSuperadminSeeder"
docker compose exec laravel bash -lc "php artisan db:seed --class=CreateSuperadminTeamSeeder"
...
フロントのビルドは `VITE_APP_BASE_PATH` によってアセットの参照パスが変わる。

| 環境 | VITE_APP_BASE_PATH | ビルド後のパス |
|---|---|---|
| ローカル | （空） | `/build/assets/...` |
| さくら | `/members` | `/members/build/assets/...` |

**さくらにデプロイする際のビルド手順:**
1. `.env` の `VITE_APP_BASE_PATH=/members` に変更
2. `docker compose exec laravel bash -lc "npm run build"`
3. `git add public/build/ && git commit && git push`
4. `.env` を `VITE_APP_BASE_PATH=` に戻す
5. さくらで `git pull && php artisan config:clear`

**ローカル開発中のフロント確認:**
```bash
docker compose exec laravel bash -lc "npm run dev"
```
Vite の開発サーバーが起動するのでビルド不要。

### npm install について

`.npmrc` に `legacy-peer-deps=true` を設定済み。
`@vitejs/plugin-vue@5.x` と `vite@7.x` のpeer dependency競合を回避するため。
`npm install` 実行時に自動適用される（フラグ不要）。

### storageのパーミッションエラーが出た場合

```bash
docker compose exec -u root laravel bash -lc "chmod -R 777 storage bootstrap/cache && chown -R www-data:www-data storage bootstrap/cache"
docker compose exec laravel bash -lc "php artisan config:clear && php artisan view:clear && php artisan cache:clear"
```
...
CI では MySQL が使えないため `phpunit.xml` で SQLite（`:memory:`）を指定している。
```xml
<env name="DB_CONNECTION" value="sqlite"/>
<env name="DB_DATABASE" value=":memory:"/>
```

**MySQL固有のマイグレーションには SQLite ガードが必要:**
```php
if (DB::getDriverName() === 'sqlite') return;
```
`MODIFY`・`DROP FOREIGN KEY`・`AUTO_INCREMENT`・`information_schema` 参照を含むマイグレーションに追加すること。
try-catchでエラーを握り潰しているマイグレーションは対応不要。

### スキップしているテスト

以下の機能はこのアプリでは未実装または無効のため、対応テストをスキップ済み:
- `routes/settings.php` — 未登録（`/settings/profile`、`/settings/password`）
- `routes/auth.php` — 未登録（`/verify-email`、`/confirm-password`）
- Fortify emailVerification — `config/fortify.php` で無効
- `project_schedule_assignments` テーブル — マイグレーション未実行
- `password_reset_tokens` テーブル — マイグレーション未実行
- DiaryFactory — 未作成

### ESLint設定

`eslint.config.js` で以下のルールをオフ/warn に設定済み（既存コードベースに合わせた緩和）:
- `vue/block-lang: off`
...
| パス | 役割 |
|---|---|
| `~/SunBWork/` | Laravel ルート（app, vendor, config 等） |
| `~/www/members/` | Web 公開ディレクトリ（Laravel の public/ 相当） |
| `~/www/members/build/` | `~/SunBWork/public/build/` へのシンボリックリンク |
| `~/www/members/index.php` | Laravel エントリポイント（パス修正済み） |

**`~/www/members/index.php` のパス（通常の Laravel と異なる）:**
```php
require __DIR__.'/../../SunBWork/vendor/autoload.php';
$app = require_once __DIR__.'/../../SunBWork/bootstrap/app.php';
```
...
**`~/www/members/.htaccess`:** 標準 Laravel の `.htaccess`（`RewriteBase /members/`）

---

### さくらレンタルサーバーの制約・注意点

- `php artisan` は SSH 接続後に `~/SunBWork/` で直接実行可能（Docker 不要）
- `nano` は使用不可。テキスト編集は `vi` を使う
- `.htaccess` に `php_flag` / `php_value` は使用不可（500 エラーになる）
- `Options +FollowSymLinks` を `~/www/.htaccess` に書くと 500 エラーになる場合がある
- Reverb / WebSocket はさくらレンタルサーバーでは動作しない（チャットのリアルタイム機能は本番では無効）
- `sed -i` は BSD 版のため `-i ''` が必要（Linux と構文が異なる）
- エラーログは `~/log/` に `error_log` が存在しない。Laravel のログは `~/SunBWork/storage/logs/laravel.log`
- `vi` でファイルがロックされる場合はスワップファイルを `find ~/path/ -name "*.sw*"` で確認・削除

---

### 本番 `.env` の重要設定（`~/SunBWork/.env`）
...
**`APP_DEBUG=true` にするとエラー詳細が表示される（トラブル時のみ一時的に使用）**
...
### Vite ビルド設定（`vite.config.js`）

`VITE_APP_BASE_PATH=/members` が `.env` に設定されている場合、ビルド時のベースパスが `/members/build/` になる。
`process.env` では `.env` ファイルを読まないため、`loadEnv` を使用していること（重要）。

```js
export default defineConfig(({ mode }) => {
    const env = loadEnv(mode, process.cwd(), '');
    const basePath = env.VITE_APP_BASE_PATH ? `${env.VITE_APP_BASE_PATH}/build/` : '/';
...
npm run build         # フロントエンドビルド（ローカルで実行後 git push → サーバーで git pull）
php artisan config:clear && php artisan cache:clear
```
...
### AssignmentForm.vue — Coordinator / User 統合コンポーネント

`AssignmentForm.vue` は **Coordinator からの依頼** と **User 自身の自己登録** の両方を `mode` prop で切り替える統一コンポーネント。
`AssignmentForm_user.vue` は廃止済み（同ファイルに統合）。
...
| 値 | 使用画面 | 保存先 |
|---|---|---|
| `'coordinator'`（デフォルト）| Coordinator のジョブ依頼フォーム | `coordinator.project_jobs.assignments.store` |
| `'user'` | User の自己登録・MyJobBox 編集 | `inertiaFetch` (PUT/PATCH/POST) |

**呼び出し側ページ一覧:**

| ページ | mode |
|---|---|
| `Coordinator/ProjectJobs/JobAssign/Create.vue` | `'coordinator'`（デフォルト省略可）|
| `JobBox/create_user.vue` | `mode="user"` |
| `MyJobBox/Create_user.vue` | `mode="user"` |
| `MyJobBox/Edit_user.vue` | `mode="user"` |
| `Events/Create_Job.vue` | `mode="user"` |
| `Events/Create copy.vue` | `mode="user"` |
| `JobBox/Edit_User.vue` | `mode="user"` |

**mode による主な違い:**

| 項目 | coordinator | user |
|---|---|---|
| クライアント/案件 | 読み取り専用表示 | 編集可能セレクト |
| ステータスフィールド | 非表示 | 表示 |
| 日付・見積時間 | 常に編集可 | `block.id` がある場合は読み取り表示 |
| 時間レンジ | 6〜22時 | 7〜23時 |
| 保存方法 | `router.post()` (Inertia) | `inertiaFetch` (axios ベース) |

**重要: Coordinator の保存は必ず `router.post()` を使うこと**

Inertia の `router.post()` は CSRF トークンを自動付与する。
`fetch` / `axios` / `inertiaFetch` に差し替えると **419 CSRF mismatch** になる。
...
// ❌ NG: inertiaFetch を使うと CSRF エラー
inertiaFetch('POST', url, payload);
```

### `desired_start_date` vs `desired_end_date` の意味（重要）

この 2 フィールドは **依頼者と受領者で意味が異なる**。比較・バリデーションしてはいけない。

| フィールド | Coordinator 依頼時 | User 自己登録時 |
|---|---|---|
| `desired_start_date` | 割当希望日（作業開始希望） | **ユーザーの作業日**（当日） |
| `desired_end_date` | 締め切り日 | （不使用 / start と同値で上書き）|

`ProjectJobAssignmentController::store()` の `desired_end_date < desired_start_date` バリデーションは **削除済み**。
User が自分のスケジュールを設定するとアラートが出るバグがあったため排除した。
`store()` では `desired_end_date` を `desired_start_date` で上書きする処理が残っている（line 74 付近）。

### ProjectJobs Show ページ（Coordinator 詳細画面）

`resources/js/Pages/Coordinator/ProjectJobs/Show.vue`

**レイアウト構成（リデザイン後）:**
- タイトル行：「クライアント名 / 案件名」の左横に 編集・ジョブ割り当て・ジョブ分析・一覧へ ボタンを `gap-5` で並置
- 詳細テキスト（`job.detail`）はカード表示
- スケジュールセクション：タイトル横に「登録/編集」「カレンダー」ボタン（左寄せ `gap-4`）
- メンバーセクション：モーダル廃止。インライン丸バッジで一覧表示。タイトル横に「登録/編集」ボタン（左寄せ）

**廃止した要素:**
- 「決定済み/未設定」ステータスボックス
- Members モーダル（`showMembersModal` ref ごと削除）
- `ref` インポート（不要になったため削除済み）

### 案件カレンダー（Coordinator タブ）

Coordinator のタブメニューに「案件カレンダー」を追加（2026-03）。

**ルート:** `GET coordinator/project_jobs/calendar` → `coordinator.project_jobs.calendar`
**コントローラー:** `app/Http/Controllers/Coordinator/ProjectJobsCalendarController.php`
**Vue ページ:** `resources/js/Pages/Coordinator/ProjectJobs/CalendarAll.vue`

**仕様:**
- ログイン中の Coordinator がオーナー（`user_id = me`）の全案件スケジュールを 1 画面に表示
- 案件ごとに 12 色パレットから自動割り当て（12件超は循環）
- 上部凡例にプロジェクト名・クライアント名・色を表示
- 「完了済み案件を非表示」チェックボックスで絞り込み
- セル高さ `min-height: 140px`（デフォルト約 2 倍）
- ラッパー `height: 560px; overflow-y: auto;` でスクロール対応
- マウント後に今日のセルを DOM 検索し、第 4 週・第 5 週なら `scrollTop = scrollHeight`（最下部へ）

**色パレット（`ProjectJobsCalendarController::PALETTE`）:**
```php
['#e11d48','#2563eb','#16a34a','#d97706','#7c3aed','#0891b2',
 '#dc2626','#65a30d','#9333ea','#0284c7','#ca8a04','#059669']
```

**タブ追加箇所:** `CoordinatorNavigationTabs.vue` — `active="calendar"` で強調
