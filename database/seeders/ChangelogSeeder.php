...
HTML,
            ...
            [
                ...
                'summary'      => '同じ受注番号で組版・可変・発送など複数の作業が発生する場合に、これまでは2件目以降を登録できず、受注番号をダミーにする・品名に「_可変」を付けるといった回避が必要でした。今回から、既存案件がある場合は確認画面が表示され、内容を確認したうえで「別作業として登録する」を選べば、そのまま登録できるようになりました。確認画面には既存案件の会社名・クライアント名・リーダー名が表示され、「別タブで開く」から中身も確認できます。グループ会社（サンエー印刷・サン・ブレーン）で同じ伝票番号を扱う場合も、どちらの会社の案件かがひと目で分かります。なお品名の重複はこれまでも確認のうえ登録できていましたが、確認画面がエラーに見えていたため、文言を分かりやすく整理しました。案件編集画面にも同じ確認画面を追加しています。',
                'design_files' => [],
                'claude_notes' => '発端は現場からの相談。受注番号 4603448 の案件が組版で登録済みのところに可変作業が回ってきたが登録できず、大本の担当者にメンバー追加してもらう運用で回避していた（担当者が休みだと詰まる）。別案件では受注番号をダミー・品名を「秋無料送り① Ａ4ハガキ 8/21投函_可変」として登録していた。' . "\n\n" . '調査の結果、ブロックしていたのは受注番号のみで、品名は Coordinator\\ProjectJobController::checkDuplicate + Create.vue の確認モーダル（forceSubmit）で以前から強行登録可能だった。現場が「品名も重複するとエラーが出る」と認識していたのは、モーダルの見た目（黄色の警告アイコン・「それでも登録する」）が禁止表示に見えていたため。' . "\n\n" . 'project_jobs.jobcode には DB のユニーク制約が無く（2025_09_07_000000 の migration は string()->nullable() のみ）、アプリ層の where()->exists() チェックだけだったため、マイグレーション不要で緩められた。store() / update() は allow_duplicate_jobcode フラグを受け取り、フラグが無ければ従来どおり弾く（フロントを迂回した直接 POST での意図しない重複登録は引き続き防ぐ）。' . "\n\n" . '当初は受注番号チェックが company_id を見ず全社横断で検索している点をマルチテナントのバグとして報告したが、ユーザーからサンエー印刷（親会社・印刷）とサン・ブレーン（データ制作）がグループ会社で、印刷伝票処理 → 組版・下版 → 印刷 と同じ伝票番号・クライアントを共有して流れる業務であることを確認。会社スコープに絞ると正しい重複まで弾いてしまうため、横断検索は維持し、代わりに結果へ same_company フラグと会社名を付けてフロントで色分け（自社=黄バッジ／他社=青バッジ）する方式にした。clients は既に company_clients 中間テーブルで会社間共有される設計になっており、方針とも整合する。' . "\n\n" . 'checkDuplicate は品名専用（client_id required）から、jobcode と title の両方を任意で受け取り duplicates / jobcode_duplicates の2系統を返す形に拡張。判定ロジックは findTitleDuplicates() / findJobcodeDuplicates() に分離し、SuperAdmin のコンテキスト切替を含む会社ID解決は resolveActorCompanyId() に切り出して store() の既存コードもそこへ寄せた。jobcode 側は with([company:id,name, user:id,name, client:id,name]) を select の外部キーと合わせて eager load し、limit 20。' . "\n\n" . 'フロントは Create.vue / Edit.vue で同じものが必要になるため、Components/DuplicateJobModal.vue（2セクション構成のモーダル）と Composables/useJobDuplicateCheck.js（fetch + モーダル状態）に共通化した。Edit.vue はこれまで事前チェックが無くサーバーエラーを表示するだけだったため、確認モーダルの流れを新規に追加（exclude_id で自分自身を除外）。ローカルDBで受注番号 000 が2件重複しているデータ（過去にチェックをすり抜けたもの）を使い、会社名・リーダー名・クライアント名まで正しく引けることを確認済み。' . "\n\n" . '未対応として残したもの: (1) ProjectJobCsvController の CSV 一括取込は重複行をスキップする挙動のまま（同じ CSV を二度取り込んでも増えない仕様と判断） (2) Prepress\\TicketController（製版チケット）は別テーブル・別業務のため据え置き。サンエー印刷側の伝票処理で同じ問題が出るようなら同方式を適用できる。',
                'body'         => <<<'HTML'
<section class="cl-problem">
  <h3>背景・問題</h3>
  <ul>
    <li>同じ受注番号で複数の作業（組版のあとに可変、印刷のあとに発送など）が発生しても、2件目以降を登録できなかった</li>
    <li>回避のため、受注番号をダミーにする・品名に「_可変」を付ける・大本の担当者に案件のメンバーへ追加してもらう、といった手間が発生していた。担当者が休みの日は登録を進められなかった</li>
    <li>品名が重複したときの確認画面が警告表示に見えており、登録できないものと受け取られていた（実際は確認のうえ登録できていた）</li>
  </ul>
</section>
...
<section class="cl-fix">
  <h3>修正・改善内容</h3>
  <ul>
    <li>受注番号が既存案件と重複する場合、エラーで止めるのではなく確認画面を表示し、「別作業として登録する」を選べばそのまま登録できるようにした</li>
    <li>確認画面に、既存案件の会社名・品名・クライアント名・リーダー名・登録月を表示し、「別タブで開く」から案件の中身も確認できるようにした</li>
    <li>グループ会社の案件は青、自社の案件は黄色のバッジで表示し、どちらの会社で登録済みかが分かるようにした</li>
    <li>品名の重複時の確認画面の文言を「閉じる（内容を修正する）」「このまま登録する」に整理し、登録できることが分かるようにした</li>
    <li>案件編集画面にも同じ確認画面を追加した（これまでは保存してからエラーが出ていた）</li>
    <li>確認画面に「品名に作業内容（例: 可変・発送）を入れておくと一覧で見分けやすくなります」の案内を追加した</li>
  </ul>
</section>
HTML,
            ],
            [
                'version'      => 'diary-department-read-all-1',
                'title'        => '日報：部署ごとに「全部既読にする」ができるようにした',
                'released_at'  => '2026-08-14',
                'summary'      => '日報の日付別表示で部署ごとに表示しているとき、「全部既読にする」ボタンを押すとその日のすべての部署の日報が既読になっていました。今回から、押した部署の日報だけが既読になります。ボタンの名前も「この部署を全部既読にする」に変更し、既読にしたあとのメッセージにも部署名が表示されるようになりました。',
                'design_files' => [],
                'claude_notes' => 'Diaries\\DiaryInteractionController::markReadAll() と DiaryManager 側の同名メソッドの2箇所に department パラメータの絞り込みを追加。Diary::with(user.department) で読み込んだうえで、部署名（department が無い場合は「未所属」）で filter してから addReadBy() する。フラッシュメッセージも部署指定時は「◯◯の日報を既読にしました」に切り替える。' . "\n\n" . 'ByDate.vue 側は部署グループごとのボタンから group.department を送るよう変更し、ラベルを「全部既読にする」→「この部署を全部既読にする」に変更した。Vue と PHP がセットで完結する変更のため、どちらか片方だけをデプロイすると部署ボタンが全部署を既読にしてしまう点に注意。',
                'body'         => <<<'HTML'
<section class="cl-problem">
  <h3>背景・問題</h3>
  <ul>
    <li>日報の日付別表示で部署ごとに分けて表示しているとき、部署見出しの横にある「全部既読にする」を押すと、その日の全部署の日報が既読になっていた</li>
  </ul>
</section>
...
<section class="cl-fix">
  <h3>修正・改善内容</h3>
  <ul>
    <li>押した部署の日報だけが既読になるようにした</li>
    <li>ボタンの名前を「この部署を全部既読にする」に変更した</li>
    <li>既読にしたあとのメッセージに部署名を表示するようにした</li>
  </ul>
</section>
HTML,
            ],
...
<section class="cl-fix">
  ...
  <ul>
    ...
    <li>部署の案件詳細ページとマイ案件の工数分析で、校正ジョブの作業時刻が9時間ずれて表示される問題を修正した</li>
    <li>朝のまとめ通知で、朝9時より前に始まる校正予定が前日扱いとなり通知されない問題を修正した</li>
