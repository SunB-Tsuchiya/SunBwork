...
## 付録：template直下（main直下）の大枠に関する厳密な決めごと

目的: ページごとの余白・背景・中央揃えを統一し、AppLayout.vue の構造と崩れない入れ子を保証するためのルールです。AI エージェントや開発者は下記を必ず守ってください。

必須ルール:

- すべてのページは `AppLayout.vue` をルートに使い、ページ固有の main コンテンツは必ず `<template #header>` と main 内の大枠で包むこと。
- main 配下の最上位ブロックは次の2つの div を必ず含めること（順序を変えない）：
...
- 上の 3 重入れ子（py-12 / max-w-7xl / rounded bg-white）構造はページ全体を包む「大枠」です。これらの閉じタグ (`</div>`) は必ず `<AppLayout>` の閉じ直前にまとめて置くこと（閉じ忘れ・途中での再オープンを禁止）。
- 途中でこの大枠の一部を閉じたり、別の場所で再度開いたりしてはいけません。これが守られないと背景色や余白、カードの境界が崩れます。

例外と注意点:

- 小さなウィジェットやカードを追加する場合は、必ず上記の大枠内に挿入すること。大枠外にはグローバルな要素（たとえば persistent banner やフローティングボタン等）以外を置かないでください。
- 管理画面などでタブナビを使う場合、ナビは大枠内の最上部（`max-w-7xl` の直下）に配置してください。例：
...
- `<div class="py-12">` と `<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">` をペアで使っているか。
- ペアの閉じタグを `<AppLayout>` の閉じ直前まで残しているか（途中で閉じていないか）。
- ページ毎のカードは `rounded bg-white p-6 shadow` など統一されたクラスを使っているか。
- レスポンシブ用の `sm:`, `md:`, `lg:` クラスを適切に使っているか。
- 直接 `<main>` タグをいじる必要がある場合は AppLayout.vue の構造を壊さないこと。

もし違反が見つかった場合の対処:

- レイアウト崩れが起きたら、まず `<AppLayout>` の中で opening/closing の `div` が正しくペアになっているか確認してください。
- それでも直らない場合は、該当ページの template を `AppLayout.vue` の構造と並べて diff を取り、どのタグが余分/不足しているかを見つけて修正してください。

このセクションはプロジェクトの UI 一貫性を守るため重要です。変更を加える場合は、必ずこのファイルを編集し、理由と変更例を記載してください。
