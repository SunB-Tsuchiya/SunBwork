...
サン・ブレーンの売上には、サンエー印刷から依頼される売上と、サン・ブレーンが独自に受注する
売上がある。Excelの列構造は同一であり、内容からは判別できないため、ファイル名だけを正本として
受注経路を判定する。

- 既存登録データはすべて「サンエー印刷経由」として後方補完する。
- 通常ファイルは「サンエー印刷経由」、basenameが正確に`_独自.xlsx`で終わるファイルは「独自受注」。
- 1ファイル内で両経路を混在させない。経理側の運用規則として統一する。
- 企画・制作・オンデマンドの3部署すべてに両経路が存在する。
- 対象期間は2020年から2026年までの既存データを含み、年次・月次・rangeの全取込形式に対応する。
- 同じ受注Noが両経路に存在する例外を許す。同一経路内の重複検証は従来どおり維持する。
- 再取込は会社・部署・受注経路・対象月単位。独自受注の再取込で通常データを切り替えない。
- 得意先名が両経路に存在する場合は同じ得意先として合算し、経路別内訳も表示する。
- 月・部署について両経路が登録されて初めて「登録完了」。片方だけなら「一部未登録」。
- 今回は0円専用ファイルの新仕様を作らない。現在の検証仕様を維持し、実際に0円期間が発生した
  時点で別途設計する。
- 対象は会社コード`SUNBRAIN`（サン・ブレーン）のみ。サンエー印刷会社側の画面・運用は変更しない。
- 既存の年次・期別Excel出力にも合計・サンエー印刷経由・独自受注の内訳を追加する。

#### 20.2 用語と内部値

既存の`sales_imports.source_type`はファイル期間種別（`annual`/`monthly`/`range`）なので、
受注経路には流用しない。新しいカラム名は`order_channel`とする。

| 内部値 | サン・ブレーンでの表示 | 意味 |
|---|---|---|
| `standard` | サンエー印刷経由 | 通常のファイル名で取り込んだ売上 |
| `direct` | 独自受注 | `_独自.xlsx`で終わるファイルから取り込んだ売上 |
| `all` | 合計 | API/UIフィルタ専用。DBには保存しない |

会社IDをハードコードしない。会社コード`SUNBRAIN`かどうかを一か所で判定する
`SalesOrderChannels`（名称は実装時に既存命名へ合わせてよい）を設け、表示ラベル・許可値・
両経路必須判定を集約する。サンエー印刷を含む他社は`standard`だけを内部的に持ち、
経路UIを表示しない。`standard`という汎用値を採るのは、他社データへ`sanei`という不自然な
内部値を保存しないためである。

#### 20.3 ファイル名判定

サン・ブレーンではファイル名をサーバー側でも必須検証する。フロント側解析だけを信用しない。
許可するbasenameは次の6形式とする（部署名は会社に登録された部署ラベルから組み立てる）。
...
概念上の正規表現は以下。実装では部署ラベルを`preg_quote`し、basenameに対してUnicode対応で判定する。

```regex
^(部署ラベル)_(\d{4})年(?:(\d{1,2})-(\d{1,2})月|(\d{1,2})月)?(_独自)?\.xlsx$
```

- `_独自`は拡張子の直前に完全一致した場合だけ`direct`とする。
- `_独自版`、`_ 独自`、`（独自）`、追加の空白・文字、`.xls`は拒否する。
- 規則外の場合は手入力による救済を行わず、ファイル名を直して再選択させる。
- サーバーがファイル名から部署・期間種別・年・月・終了月・経路を再解析する。
- リクエストに現在のフォーム値を残す場合も、サーバー解析値との完全一致を要求する。
  より安全で単純なら、サーバー解析値だけを正としてフォーム値を上書きしてよい。
- プレビューキャッシュへ`order_channel`を保存し、確定時はキャッシュ値だけを使う。
- プレビュー画面の確定ボタン付近へ「受注経路: 独自受注」等を目立つバッジで表示する。

サンエー印刷会社側については今回の対象外なので、現在のファイル名運用を壊さない。
厳格判定の適用条件を会社コードで明示する。

#### 20.4 DB変更

すべて`sales`接続のmigrationとして実装する。

`sales_imports`:

- `order_channel varchar(16)`を追加。
- 既存行をすべて`standard`へ後方補完後、NOT NULL・default `standard`とする。
- 推奨indexを`[company_id, department_key, order_channel, source_year, source_month, version]`へ拡張。
- `[company_id, file_sha256]`の一意制約は維持する。同一内容をファイル名だけ変えて別経路へ
  二重登録することも防ぐ。
...
- `order_channel varchar(16)`を追加。
- 既存行をすべて`standard`へ後方補完後、NOT NULL・default `standard`とする。
- 現在のunique `[company_id, department_key, sales_year, sales_month]`を、
  `[company_id, department_key, order_channel, sales_year, sales_month]`へ変更する。
- index名はMySQLの64文字制限内に収める。

`sales_orders`/`sales_order_details`へは`order_channel`を重複保存しない。1ファイル1経路が保証されるため、
`sales_imports`または`sales_active_months`から取得する。検索性能上の問題が計測で確認されるまでは、
非正規化による不整合リスクを増やさない。

migrationの`down()`で両経路のactive行が同じ会社・部署・年月に存在する場合、旧uniqueへそのまま
戻せない。行を勝手に削除せず、衝突を検出して明示的な例外で停止する。ロールバックのために
`direct`データを自動削除してはならない。

#### 20.5 取込・版管理・重複検証

- `SalesImportService::nextVersion()`の採番条件へ`order_channel`を加え、両経路で独立採番する。
- `SalesImportService::persistImport()`は`sales_imports`と`sales_active_months`の両方へ同じ
  `order_channel`を保存する。
- `SalesActiveMonth::updateOrCreate()`の探索キーへ`order_channel`を加える。
- `calculateDiff()`は同じ会社・部署・経路の旧activeだけと比較する。
- `SalesImportValidator::checkCrossMonthDuplicates()`は同じ会社・部署・経路内だけを重複対象とする。
  `standard`と`direct`に同一受注Noがあってもエラーにしない。
- ファイル内の受注組立、N列の最終行金額、負額、除外受注等の既存規則は変更しない。
- 監査ログcontextと取込完了レスポンスに`order_channel`を含める。ただし得意先名、品名、金額明細は
  ログへ書かない。
- 取込履歴へ受注経路を表示し、会社・部署・期間・経路が同時に分かるようにする。

#### 20.6 集計サービスとAPI契約

`SalesQueryService::activeOrdersQuery()`を受注経路対応の唯一の起点にする。
すべての集計が個別にJOIN条件を持たないようにし、既存の`forCompany($companyId)`安全装置は維持する。

- APIの共通フィルタとして`order_channel=all|standard|direct`を追加し、未指定は`all`。
- サン・ブレーン以外は`standard`だけを許可し、`direct`指定は422とするかUIから送信させない。
- `all`は両方のactiveデータを合算する。`standard`/`direct`は該当経路だけに絞る。
- 金額を返す主要な集計行は、可能な範囲で次の共通形を持つ。

```json
{
  "amount": 1000000,
  "standard_amount": 700000,
  "direct_amount": 300000,
  "direct_share": 30.0,
  "registration": "complete"
}
```

- `amount = standard_amount + direct_amount`を常に保証する。
- `direct_share = direct_amount / amount * 100`。合計が0以下、または負額調整により構成比として
  誤解を招く場合は`null`を返し、画面は`—`とする。
- 片方の経路が未登録の場合は0円と断定しない。登録済み分の暫定合計は表示してよいが、
  `registration=partial`を返し「一部未登録のため暫定値」と明示する。
- 両経路とも未登録は従来どおり`no_data`。両方登録済みなら、片方の対象範囲内売上が0件でも
  active pointerがある限り`complete`と判定できる（ただし0円ファイル作成規則は今回変更しない）。
- 前月比・前年比・増減率は比較元と比較先がともに`complete`の場合を正式値とする。partialを含む場合は
  数字を隠すか「暫定」表示にし、通常の増減色で断定的に評価しない。

受注経路対応は次の全既存集計へ伝播させる。

- 月次、年次、期別、同月比較、左右比較
- 5年推移・3年/5年重ね表示・累計
- 得意先、商品、分類、項目、品名検索
- 得意先詳細・商品詳細・新規/取扱終了・増減ランキング
- 最新登録月/年の判定、登録状況、ファイル一覧

得意先統合は従来どおり会社単位であり、経路別にグループを二重管理しない。同一得意先は合算した
表示名で集計し、各行へ`standard_amount`/`direct_amount`/`direct_share`を付ける。

#### 20.7 登録状況画面

サン・ブレーンでは部署×年月セルを、単一activeの有無ではなく両経路で判定する。

| standard | direct | 状態 |
|---|---|---|
| なし | なし | 未登録 |
| あり | なし | 一部未登録（独自受注なし） |
| なし | あり | 一部未登録（サンエー印刷経由なし） |
| あり | あり | 登録完了 |

セルまたは詳細ポップオーバーには、各経路の登録有無・金額・取込ファイル・版を表示する。
年別ファイル一覧にも`受注経路`列を追加する。画面上の「登録完了件数」は両方がそろった月だけを数える。
移行直後は既存`standard`だけなので一時的に「一部未登録」となるが、これはデータ消失ではなく
独自ファイル取込待ちである旨を案内する。

#### 20.8 分析画面の表示方針

レイアウト細部はClaude Codeが既存UI規則に合わせて決めてよい。ただし、単なる表の列追加ではなく、
次の情報階層を守る。

1. 初期表示は「合計」。KPIに合計、サンエー印刷経由、独自受注、独自比率を表示。
2. `合計 / サンエー印刷経由 / 独自受注`の切替を期間・部署フィルタ付近へ置き、URLクエリへ保存。
3. 構成を見るグラフは積み上げ棒を基本とし、`standard`を青、`direct`を橙系の固定色で全画面統一。
4. ツールチップには合計・両経路金額・独自比率・登録状態を表示。
5. ランキングは横棒の区間を両色で分け、同じ得意先・商品を別行に分裂させない。
6. 表は合計を主列とし、サンエー印刷経由・独自受注・独自比率を補助列にする。上位10/20件と
   詳細展開の既存方針を維持する。
7. partial期間には画面上部と該当グラフへ警告を表示し、未登録を0円に見せない。

画面別の推奨表現:

- 月次: 13か月推移を積み上げ棒、当月KPIを4分割、前月/前年同月カードにも内訳を表示。
- 年次・期別: 月別積み上げ棒＋合計推移。過去3年/5年の重ね線は総額比較を主にし、選択年の
  経路構成は別の積み上げ表示またはツールチップで示して過密化を避ける。
- 同月比較: 年ごとの積み上げ棒で、突出年がどちらの経路によるものか分かるようにする。
- 左右比較: A/Bそれぞれを積み上げ棒にし、差額に加えて独自比率差も表示。
- 得意先・商品分析: 合算順位を維持し、棒・詳細推移・受注一覧へ経路表示を加える。

色だけに依存せず、凡例・ラベル・ツールチップへ必ず「サンエー印刷経由」「独自受注」を書く。

#### 20.9 Excel出力

対象は現在実装済みの年次分析・期別分析のExcel出力。新しい出力画面は追加しない。

- サマリー: 合計、サンエー印刷経由、独自受注、独自比率、登録状態。
- 月別: 各月の合計・両経路・独自比率。partial月は状態列で明示。
- 得意先/分類/項目: 合計・両経路・独自比率。
- 受注一覧: `受注経路`列を追加。受注Noだけで一意と仮定せず、経路と組み合わせて扱う。
- 画面の部署・年・得意先統合・経路フィルタを出力へ反映する。
- 数式インジェクション防止、非公開一時出力、レスポンス後削除等の既存保護を維持する。

#### 20.10 実装順序

1. migration・モデル・`SalesOrderChannels`・後方補完。
2. サーバー側ファイル名解析とプレビュー/確定/版管理/重複検証。
3. active queryと共通集計レスポンス、partial判定。
4. 登録状況・取込履歴。
5. 月次→年次→期別→同月→左右比較の順に可視化。
6. 得意先・商品・分類・項目・検索。
7. 年次/期別Excel出力。
8. 全SalesAnalysisテスト、全体テスト、Vite build、文書・Changelog更新。

#### 20.11 Phase 20受入基準

- 既存データがすべて`standard`として保持され、金額・active pointerが変わらない。
- `_独自.xlsx`だけが`direct`になり、誤った接尾辞や手入力回避では確定できない。
- 同じ会社・部署・年月にstandard/directが同時にactiveとして存在できる。
- direct再取込でstandardのactive pointerが変わらず、その逆も同様。
- 同一受注Noが両経路にあっても取込可能で、同一経路内の他月重複は検出される。
- 合計が両経路の和と一致し、月次/年次/期別/比較/得意先/商品へ同じ結果が伝播する。
- 同名得意先は1件に合算され、その内訳が正しい。
- 片方未登録を0円と扱わず、登録状況・分析・比較・Excelでpartialと分かる。
- サンエー印刷会社側の既存取込・分析・登録完了判定が変わらない。
- 年次/期別Excelの合計・内訳が画面と一致する。
- 本番sales DBを照会せず、架空fixtureだけで検証する。
