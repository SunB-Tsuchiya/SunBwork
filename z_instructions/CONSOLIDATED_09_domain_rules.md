...
- 校正予約は、通常校正依頼の `proof_requests` ではなく `proof_reservations` に保存する。
- 予約を作成しただけでは校正依頼受信箱・校正ジョブ管理へ追加しない。
- `requested_at_mode` / `deadline_mode` がともに `datetime` で開始 < 終了の場合のみ、校正カレンダー登録を許可する。
- 自由記述の予定を日時へ推測変換しない。
- 同一案件の予約は、タイトル一致、または依頼予定日・締め切り日の組み合わせ一致（JSTの日付単位、時間無視）で重複候補とする。
- 重複候補は自動削除せず、ユーザーが確認ダイアログで承認した場合のみ保存する。
- 校正予約の「削除」は物理削除ではなく `status=deleted` とし、監査可能な履歴を残す。
