...
{
    /** 部署を問わず、有効データに含まれる得意先名の受注を対象にする基底クエリ */
    private function allActiveOrdersQuery(): Builder
    {
        return SalesOrder::query()
            ->join('sales_active_months', function ($join) {
                $join->on('sales_orders.sales_import_id', '=', 'sales_active_months.sales_import_id')
                    ...
                    ->whereIn('sales_active_months.department_key', SalesDepartments::ENABLED_KEYS);
            });
    }
    ...
    {
        $groupedNames = SalesClientGroupMember::pluck('client_name');
    ...
    {
        return SalesClientGroup::with('members')->orderBy('name')->get()
            ->map(fn ($g) => [
                'id' => $g->id,
                'name' => $g->name,
