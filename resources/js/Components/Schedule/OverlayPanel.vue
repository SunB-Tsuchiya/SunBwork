...
const props = defineProps({
    overlays:    { type: Array, default: () => [] }, // 現在のオーバーレイ一覧
...
const pickerTab    = ref('user');   // 'user' | 'company' | 'dept'
...
watch(pickerTab, () => { filterCoId.value = ''; filterDeptId.value = ''; });

function alreadyAdded(type, id) {
    if (type === 'user')    return props.overlays.some(o => String(o.target_user_id) === String(id));
    if (type === 'company') return props.overlays.some(o => String(o.target_company_id) === String(id));
    if (type === 'dept')    return props.overlays.some(o => String(o.target_department_id) === String(id));
    return false;
}

async function openPicker() {
    pickerTab.value    = 'user';
    filterCoId.value   = '';
    filterDeptId.value = '';
...
async function addUserOverlay(userId) {
    if (alreadyAdded('user', userId) || adding.value) return;
    adding.value = true;
    try {
...
async function addCompanyOverlay(companyId) {
    if (alreadyAdded('company', companyId) || adding.value) return;
    adding.value = true;
    try {
        const res = await axios.post(
            route('schedule.overlays.store'),
...
async function addDeptOverlay(deptId) {
    if (alreadyAdded('dept', deptId) || adding.value) return;
    adding.value = true;
    try {
        const res = await axios.post(
            route('schedule.overlays.store'),
...
function chipLabel(o) {
    ...
    if (o.target_company)    return `🏢 ${o.target_company.name}`;
    if (o.target_department) return `👥 ${o.target_department.name}`;
    return '?';
}
function chipClass(o) {
    if (o.target_user_id)       return 'bg-blue-100 text-blue-800';
    if (o.target_company_id)    return 'bg-green-100 text-green-800';
    if (o.target_department_id) return 'bg-orange-100 text-orange-800';
        ...
        <div v-if="showPicker" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative z-10 flex w-full max-w-lg flex-col rounded-xl bg-white shadow-2xl"
                ...
                <div class="flex-1 overflow-y-auto">
                            ...
                            <table class="min-w-full divide-y divide-gray-100 text-sm">
                                <tbody class="divide-y divide-gray-50 bg-white">
                                    <tr v-for="u in filteredUsers" :key="u.id"
                                        ...
                                        :class="alreadyAdded('user', u.id) ? 'opacity-40' : 'cursor-pointer'"
                                        ...
                                        <td class="px-3 py-2 text-right">
                                            <span v-if="alreadyAdded('user', u.id)" class="text-xs text-gray-400">追加済み</span>
                    ...
                    <!-- 会社タブ -->
                    <template v-else-if="pickerTab === 'company'">
                        <table class="min-w-full divide-y divide-gray-100 text-sm">
                            <tbody class="divide-y divide-gray-50 bg-white">
                                <tr v-for="c in companies" :key="c.id"
                                    class="hover:bg-green-50"
                                    :class="alreadyAdded('company', c.id) ? 'opacity-40' : 'cursor-pointer'"
                                    @click="addCompanyOverlay(c.id)">
                                    <td class="px-4 py-3 font-medium text-gray-900">🏢 {{ c.name }}</td>
                                    <td class="px-3 py-3 text-right">
                                        <span v-if="alreadyAdded('company', c.id)" class="text-xs text-gray-400">追加済み</span>
                                        <span v-else class="text-xs font-medium text-green-600">＋ 追加</span>
                                    </td>
                                </tr>
                                <tr v-if="!companies.length">
                                    <td colspan="2" class="py-8 text-center text-sm text-gray-400">会社が登録されていません</td>
                                </tr>
                            </tbody>
                        </table>
                    </template>
                        ...
                        <table class="min-w-full divide-y divide-gray-100 text-sm">
                            <tbody class="divide-y divide-gray-50 bg-white">
                                <template v-for="d in departments" :key="d.id">
                                    <tr v-if="!filterCoId || String(d.company_id) === filterCoId"
                                        class="hover:bg-orange-50"
                                        :class="alreadyAdded('dept', d.id) ? 'opacity-40' : 'cursor-pointer'"
                                        @click="addDeptOverlay(d.id)">
                                        <td class="px-4 py-2.5 font-medium text-gray-900">👥 {{ d.name }}</td>
                                        <td class="px-2 py-2.5 text-xs text-gray-400">
                                            {{ companies.find(c => c.id === d.company_id)?.name ?? '' }}
                                        </td>
                                        <td class="px-3 py-2.5 text-right">
                                            <span v-if="alreadyAdded('dept', d.id)" class="text-xs text-gray-400">追加済み</span>
                                            <span v-else class="text-xs font-medium text-orange-600">＋ 追加</span>
                                        </td>
                                    </tr>
                                </template>
                                <tr v-if="!departments.length">
                                    <td colspan="3" class="py-8 text-center text-sm text-gray-400">部署が登録されていません</td>
                                </tr>
                            </tbody>
                        </table>
                    </template>
