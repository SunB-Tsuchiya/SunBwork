                ...
                >
                    ...
                    <div class="flex-1 px-2 py-2">
                        ...
                        <div
                            ...
                            :key="ev.id"
                            class="mb-1 inline-block rounded border px-2 py-0.5 text-xs font-medium"
                            :style="chipStyle(ev)"
                        >
                            <span v-if="!ev.allDay" class="mr-1 opacity-70">{{ formatTime(ev.start) }}</span>
                            {{ ev.title }}
                        </div>
                    </div>
                </div>
...
import { CLERK_EVENT_COLORS, CLERK_DEFAULT_COLOR_KEY } from '@/Components/Clerk/clerkEventColors';
...
function chipStyle(ev) {
    const color = CLERK_EVENT_COLORS[ev.color_key] || CLERK_EVENT_COLORS[CLERK_DEFAULT_COLOR_KEY];
    const style = {
        backgroundColor: color.hex + '26', // 15% alpha
...
function eventDateRange(ev) {
    ...
    if (ev.end) {
        if (ev.allDay) {
            // FullCalendar allDay end は exclusive: 1日引く
...
async function fetchPosts() {
    ...
    try {
        const res = await axios.get(route('clerk.calendar.week_posts.index'), {
            params: { year: currentYear.value, week: currentWeek.value },
        });
        flatPosts.value = res.data;
    } catch (e) {
        console.error('[ClerkWeekPlanner] fetchPosts error', e);
    } finally {
...
async function submitPost() {
    ...
    try {
        const res = await axios.post(
            route('clerk.calendar.week_posts.store'),
        ...
        console.error('[ClerkWeekPlanner] submitPost error', e);
    } finally {
...
async function submitReply(targetItem) {
    ...
    try {
        ...
        console.error('[ClerkWeekPlanner] submitReply error', e);
    } finally {
