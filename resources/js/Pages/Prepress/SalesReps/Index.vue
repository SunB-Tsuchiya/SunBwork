...
const createCompanyMode = ref('preset'); // 'preset' | 'custom'

// 全角スペース→半角・連続スペース→1つ・前後trim
...
function submitCreate() {
    ...
    router.post(route('prepress.sales_reps.store'), payload, {
        preserveScroll: true,
        onSuccess: () => {
...
const bulkResult   = ref(null); // { created, skipped } or null
...
const bulkInternalDups = computed(() => {
    const seen = new Set();
    const dups = new Set();
    for (const n of bulkNames.value) {
...
const bulkNewCount = computed(() =>
    [...new Set(bulkNames.value)].filter(
        n => !bulkDbDups.value.has(n)
    ).length
);
...
async function executeBulkStore() {
    ...
    try {
        const res = await axios.post(route('prepress.sales_reps.bulkStore'), {
...
<template>
    <AppLayout title="営業担当管理">
        ...
        <template #tabs>
            <PrepressNavigationTabs active="sales_reps" />
        </template>
                    ...
                    <div>
                        ...
                        <template v-else>
                            <textarea
                                ...
                                class="w-full rounded border border-gray-300 px-2 py-1 text-sm font-mono resize-y focus:outline-none focus:ring-1 focus:ring-indigo-400"
                            ></textarea>
                            <!-- プレビューチップ -->
                            <div v-if="bulkNames.length > 0" class="mt-2 flex flex-wrap gap-1">
                                <span
                                    v-for="(name, idx) in bulkNames"
                                    :key="idx"
                                    class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium"
                                    :class="bulkDbDups.has(name)
                                        ? 'bg-red-100 text-red-700'
                                        : bulkInternalDups.has(name)
                                            ? 'bg-yellow-100 text-yellow-700'
                                            : 'bg-green-100 text-green-700'"
                                    :title="bulkDbDups.has(name) ? 'DB登録済み（スキップ）' : bulkInternalDups.has(name) ? 'テキスト内重複' : '登録OK'"
                                >{{ name }}</span>
                            </div>
                            <!-- 凡例 -->
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <template v-for="(rep, idx) in filteredReps" :key="rep.id">
                            ...
                            <tr v-if="editingId !== rep.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2">
                                    <button @click="startEdit(rep)"
                                        class="rounded bg-yellow-500 px-2 py-0.5 text-xs text-white hover:bg-yellow-600 mr-1">
                                        編集
                                    </button>
                                    <button @click="destroy(rep)"
                            ...
                            <tr v-else class="bg-yellow-50">
                                ...
                                <td class="px-4 py-2 space-y-1">
                                    <select v-model="editCompanyMode"
                                ...
                                <td class="px-4 py-2">
                                    <button @click="submitEdit(rep)"
                                        class="rounded bg-green-700 px-2 py-0.5 text-xs text-white hover:bg-green-800 mr-1">
                                        保存
                                    </button>
