  ...
  for (const f of dt.files) {
    ...
    (async () => {
      ...
      const idx = messages.value.findIndex(m => m._tmpId === tempId);
        ...
        try { real = normalizeMessage(real) || real; } catch (e) { console.warn('normalizeMessage failed for upload', e); }
        if (idx >= 0) {
...
function openFileModal(file) {
  ...
  if (file.mime && typeof file.mime === 'string' && file.mime.startsWith('text/')) {
    ...
    fetch(streamUrl, { credentials: 'same-origin' })
      ...
      .then(txt => { fileModal.value.file.text = txt; })
      .catch(() => { fileModal.value.file.text = null; });
  }
}
function closeFileModal() { fileModal.value.open = false; fileModal.value.file = null; }
  ...
  nextTick(() => {
    ...
    const doScroll = () => {
      try {
        ...
        try { area.scrollTop = area.scrollHeight; } catch(e) {}
      }
    };
...
function normalizeMessage(msg) {
  ...
  if (msg.type === 'file' && msg.file) {
    ...
    try { prefetchFilePreview(msg.file); } catch (e) {}
    return msg;
  }
...
onMounted(() => {
  // 初期履歴をセット
  messages.value = props.messages ? (props.messages.map(m => {
      ...
      .listen('ChatMessageSent', (e) => {
        ...
        if (e.chat_room_id === props.room.id) {
          ...
          if (!messages.value.some(m => m.id === e.id)) {
            ...
            try { pushed = normalizeMessage(pushed) || pushed; } catch (err) { console.warn('normalizeMessage failed', err); }
  ...
  try {
    ...
    if (Array.isArray(res.data)) {
      ...
      const mapped = res.data.map(m => {
        try { m = normalizeMessage(m) || m; } catch (e) { /* ignore */ }
        if (m.type === 'file' && m.file && !m.file.streamUrl) {
          m.file.streamUrl = buildStreamUrl(m.file);
        }
        return m;
...
<template>
  ...
  <div ref="messageArea" class="flex-1 overflow-y-auto border rounded p-4 mb-4 bg-gradient-to-b from-white to-gray-50 border-gray-200 shadow-inner"
    @drop.prevent="handleDrop" @dragover.prevent="handleDragOver" @dragleave.prevent="handleDragLeave">
          ...
          <template v-else>
            <div v-for="(msg, idx) in messages" :key="msg.id" :ref="el => { if (!lastMessageRef) return; if (idx === messages.length - 1) { lastMessageRef.value = el } else if (lastMessageRef.value === el) { lastMessageRef.value = null } }" :data-last="idx === messages.length - 1"
              class="mb-3 flex" :class="msg.user_id === user.id ? 'justify-start' : 'justify-end'">
              <div class="flex items-end" :class="msg.user_id === user.id ? 'flex-row' : 'flex-row-reverse'">
                <div v-if="msg.user_name" :class="['flex-shrink-0', msg.user_id === user.id ? 'mr-3' : 'ml-3']">
                  <div class="w-8 h-8 rounded-full bg-indigo-100 text-indigo-800 flex items-center justify-center font-semibold text-sm">
                    {{ (msg.user_name || '').charAt(0).toUpperCase() }}
                  </div>
                </div>
                <div :class="msg.user_id === user.id ? 'chat-bubble-own' : 'chat-bubble-other'" class="relative chat-bubble">
                  <div class="relative mb-1">
                    <span v-if="!msg.is_read && msg.user_id !== user.id" class="absolute -top-2 -right-2 px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-xxs">未読</span>
                  </div>
                  <div class="break-words text-sm leading-relaxed">
                    <template v-if="msg.type === 'file' && msg.file">
                      <div class="flex items-center gap-3">
                        <div class="w-36 h-36 bg-gray-100 flex items-center justify-center rounded overflow-hidden border relative">
                          <template v-if="msg.uploading">
                            <div class="flex flex-col items-center justify-center w-full h-full">
                              <svg class="animate-spin h-6 w-6 text-gray-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path></svg>
                              <div class="text-xs text-gray-600 mt-2">ファイルをアップロードしています…</div>
                              <div class="w-20 bg-gray-200 rounded-full mt-2 overflow-hidden">
                                <div class="bg-blue-600 h-1" :style="{ width: (msg._progress || 0) + '%' }"></div>
                              </div>
                            </div>
                          </template>
                          <template v-else>
                            <template v-if="msg.file.mime && msg.file.mime.startsWith('image/')">
                              <img :src="msg.file.streamUrl || msg.file.url" class="object-cover w-full h-full cursor-pointer" @click="openFileModal(msg.file)" />
                            </template>
                            <template v-else-if="msg.file.mime && msg.file.mime === 'application/pdf'">
                              <div class="w-full h-full flex items-center justify-center text-sm text-gray-700 cursor-pointer" @click="openFileModal(msg.file)">
                                PDF プレビュー
                              </div>
                            </template>
                            <template v-else-if="msg.file.mime && msg.file.mime.startsWith('text/')">
                              <div class="w-full h-full p-2 text-sm text-gray-700 cursor-pointer" @click="openFileModal(msg.file)">
                                <pre class="text-xs whitespace-pre-wrap max-h-24 overflow-auto">{{ msg.file.original_name }}</pre>
                              </div>
                            </template>
                            <template v-else>
                              <div class="text-xs text-gray-600 px-2">{{ msg.file.original_name }}</div>
                            </template>
                          </template>
                        </div>
                        <div class="flex-1">
                          ...
                          <div class="mt-2">
                            <a :href="msg.file.streamUrl || ('/chat/attachments?path=' + encodeURIComponent(msg.file.path || msg.file.url || msg.file.thumb_path || msg.file.original_name))" target="_blank" class="text-blue-600 underline mr-3">開く</a>
                            <a :href="msg.file.streamUrl || ('/chat/attachments?path=' + encodeURIComponent(msg.file.path || msg.file.url || msg.file.thumb_path || msg.file.original_name))" :download="msg.file.original_name" class="text-gray-600">ダウンロード</a>
                          </div>
                        </div>
                      </div>
                    </template>
                  ...
                  <div class="text-xs text-gray-400 self-end mt-2 text-right">{{ formatTimestamp(msg.created_at) }}</div>
                </div>
              </div>
            </div>
          </template>
  ...
  <!-- File preview modal -->
  <div v-if="fileModal.open" class="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-60">
    <div class="bg-white rounded shadow-lg max-w-3xl w-full mx-4 p-4">
      <div class="flex justify-between items-center mb-4">
        <div class="font-bold">{{ fileModal.file?.original_name || 'プレビュー' }}</div>
        <div>
          <a :href="fileModal.file?.streamUrl || fileModal.file?.url" :download="fileModal.file?.original_name" class="mr-3 text-sm text-gray-700">ダウンロード</a>
          <button @click="closeFileModal" class="text-gray-700 px-3 py-1 border rounded">閉じる</button>
        </div>
      </div>
      <div class="max-h-[70vh] overflow-auto">
        <template v-if="fileModal.file">
          <template v-if="fileModal.file.mime && fileModal.file.mime.startsWith('image/')">
            <img :src="fileModal.file.streamUrl || fileModal.file.url" class="w-full h-auto object-contain" />
          </template>
          <template v-else-if="fileModal.file.mime && fileModal.file.mime === 'application/pdf'">
            <iframe :src="fileModal.file.streamUrl" class="w-full h-[70vh]" frameborder="0"></iframe>
          </template>
          <template v-else-if="fileModal.file.mime && fileModal.file.mime.startsWith('text/')">
            <div class="p-4 overflow-auto max-h-[70vh] whitespace-pre-wrap font-mono text-sm text-gray-800">{{ fileModal.file.text || '読み込み中...' }}</div>
          </template>
          <template v-else>
...
.chat-bubble-own {
  ...
  box-shadow: 0 6px 18px rgba(99, 102, 241, 0.06), inset 0 1px 0 rgba(255,255,255,0.6);
}
.chat-bubble-other {
  ...
  box-shadow: 0 6px 18px rgba(14, 165, 233, 0.04), inset 0 1px 0 rgba(255,255,255,0.6);
}
...
.chat-bubble-own::after, .chat-bubble-other::after {
  content: '';
  position: absolute;
  bottom: 8px;
  width: 12px;
...
/* 吹き出し矢印をアイコン側へ寄せる */
.chat-bubble-own::after { left: -6px; background: linear-gradient(180deg, #fbf7ff 0%, #f3ebff 100%); border-left:1px solid rgba(128,64,255,0.12); border-bottom:1px solid rgba(128,64,255,0.06); }
.chat-bubble-other::after { right: -6px; background: linear-gradient(180deg, #fff 0%, #f8fbff 100%); border-right:1px solid rgba(59,130,246,0.08); border-bottom:1px solid rgba(59,130,246,0.03); }
