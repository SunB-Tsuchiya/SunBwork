...
const USER_ROLE_OPTIONS = [
    { value: 'leader',      label: 'リーダー',  description: 'チーム管理・各種管理機能にアクセス可能' },
    { value: 'coordinator', label: '進行管理',  description: '案件・割当管理にアクセス可能' },
...
const submit = () => {
    form.post(route('leader.user_management.store'), {
...
<template>
    <AppLayout title="新規ユーザー登録（部署）">
        <template #tabs>
            <LeaderNavigationTabs active="user_management" />
        </template>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-base sm:text-xl font-semibold leading-tight text-gray-800">新規ユーザー登録（部署）</h2>
        ...
        <div class="rounded bg-white px-4 py-6 sm:p-6 shadow">
            <p class="mb-6 text-sm text-gray-500">あなたの部署にユーザーを登録します。会社・部署は自動設定されます。</p>
