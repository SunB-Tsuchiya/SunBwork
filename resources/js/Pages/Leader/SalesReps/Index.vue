...
function submitCreate() {
    ...
    router.post(route('leader.sales_reps.store'), payload, {
        preserveScroll: true,
        onSuccess: () => {
...
function submitEdit(rep) {
    ...
    router.patch(route('leader.sales_reps.update', { salesRep: rep.id }), payload, {
        preserveScroll: true,
...
function destroy(rep) {
    ...
    router.delete(route('leader.sales_reps.destroy', { salesRep: rep.id }), {
        preserveScroll: true,
...
async function saveOrder() {
    ...
    try {
        await axios.post(route('leader.sales_reps.reorder'), {
...
async function executeBulkStore() {
    ...
    try {
        const res = await axios.post(route('leader.sales_reps.bulkStore'), {
...
<template>
    <AppLayout title="営業担当管理">
        <template #header>
            <h2 class="text-xl font-semibold text-orange-800">営業担当管理</h2>
        </template>
        <template #tabs>
            <LeaderNavigationTabs active="sales_reps" />
        </template>
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <input v-model="searchQ" type="text" placeholder="氏名・会社で絞り込み"
                    class="rounded border border-gray-300 px-3 py-1.5 text-sm w-60 focus:outline-none focus:ring-1 focus:ring-orange-400" />
                <button @click="showCreateForm = !showCreateForm; bulkMode = false; bulkText = ''; bulkResult = null"
                    class="rounded bg-orange-700 px-4 py-1.5 text-sm text-white hover:bg-orange-800">
                    + 新規登録
                </button>
            ...
            <!-- 新規登録フォーム -->
            <div v-if="showCreateForm" class="rounded border border-orange-200 bg-orange-50 p-4 space-y-3">
                <p class="text-sm font-semibold text-orange-800">新規営業担当登録</p>
                ...
                <div v-if="!bulkMode" class="flex gap-2">
                    <button @click="submitCreate" :disabled="!createForm.name"
                        class="rounded bg-orange-700 px-4 py-1.5 text-sm text-white hover:bg-orange-800 disabled:opacity-50">登録</button>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <template v-for="(rep, idx) in filteredReps" :key="rep.id">
                            ...
                            <tr v-else class="bg-yellow-50">
                                ...
                                <td class="px-4 py-2">
                                    <button @click="submitEdit(rep)" class="rounded bg-orange-700 px-2 py-0.5 text-xs text-white hover:bg-orange-800 mr-1">保存</button>
