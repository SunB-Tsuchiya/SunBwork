...
const props = defineProps({
    ...
    meetingDefinition:  { type: Object, default: null },
});

const isEdit = computed(() => !!props.meetingDefinition);
...
const filteredMembers = computed(() => {
    ...
    if (!q) return props.availableMembers;
    return props.availableMembers.filter((u) => u.name.toLowerCase().includes(q));
});
...
function selectAll() {
    selectedMemberIds.value = filteredMembers.value.map((u) => u.id);
}
function clearAll() {
...
<template>
    <AppLayout :title="isEdit ? '会議設定 編集' : '会議設定 追加'">
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-xl font-semibold text-gray-800">{{ isEdit ? '会議設定 編集' : '会議設定 追加' }}</h2>
            </div>
        </template>
            ...
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="flex gap-4">
                        <label v-for="opt in recurrenceOptions" :key="opt.value" class="flex items-center gap-1.5 cursor-pointer text-sm">
                            <input type="radio" :value="opt.value" v-model="form.recurrence" class="accent-orange-600" />
                            {{ opt.label }}
                        </label>
                    </div>
                    <p v-if="form.errors.recurrence" class="mt-1 text-xs text-red-500">{{ form.errors.recurrence }}</p>
                </div>
                ...
                <div>
                    ...
                    <div class="flex gap-3">
                        <label v-for="opt in dayOfWeekOptions" :key="opt.value" class="flex items-center gap-1 cursor-pointer text-sm">
                            <input type="radio" :value="opt.value" v-model="form.day_of_week" class="accent-orange-600" />
                            {{ opt.label }}
                        </label>
                    </div>
                ...
                <div>
                    ...
                    <div class="mb-2 flex flex-wrap gap-1">
                        <span
                            v-for="name in selectedMemberNames"
                            :key="name"
                    ...
                    <button
                        ...
                        class="rounded border border-orange-400 px-3 py-1.5 text-sm text-orange-600 hover:bg-orange-50"
                    >
                        メンバーを選択
                    </button>
                ...
                <div class="flex gap-3 pt-2">
                    <button
                        ...
                        class="rounded bg-orange-600 px-5 py-2 text-sm font-medium text-white hover:bg-orange-700 disabled:opacity-50"
                    >
                        {{ form.processing ? '保存中...' : (isEdit ? '更新する' : '登録する') }}
                    </button>
                    <Link
                        :href="route('leader.meeting_definitions.index')"
                        class="rounded border px-5 py-2 text-sm text-gray-600 hover:bg-gray-50"
                    >
                        キャンセル
                    </Link>
                </div>
        ...
        <div v-if="showMemberModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div class="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
                <h3 class="mb-4 text-base font-semibold text-gray-800">メンバーを選択</h3>
                ...
                <div class="mb-2 flex gap-3">
                    <button type="button" @click="selectAll" class="text-xs text-orange-600 hover:underline">全選択</button>
