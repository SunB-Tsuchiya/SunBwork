 ...
 * Create.vue と同一ロジック（meetingDefinition prop があれば編集モードになる）
 */
        ...
        <div class="mx-auto max-w-2xl rounded bg-white p-6 shadow">
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <div>
                    ...
                    <button type="button" @click="showMemberModal = true" class="rounded border border-orange-400 px-3 py-1.5 text-sm text-orange-600 hover:bg-orange-50">メンバーを選択</button>
        ...
        <div v-if="showMemberModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div class="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
                    ...
                    <table class="w-full text-sm">
                        <tbody>
                            <tr v-for="member in filteredMembers" :key="member.id" class="border-b hover:bg-gray-50 cursor-pointer" @click="toggleMember(member.id)">
                                <td class="py-2 pl-3"><input type="checkbox" :checked="selectedMemberIds.includes(member.id)" @click.stop="toggleMember(member.id)" class="accent-orange-600" /></td>
                ...
                <div class="mt-4 flex gap-3">
                    <button type="button" @click="confirmMembers" class="rounded bg-orange-600 px-4 py-2 text-sm text-white hover:bg-orange-700">確定（{{ selectedMemberIds.length }}名）</button>
