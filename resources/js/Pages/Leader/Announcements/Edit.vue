...
function submit() {
    ...
    fetch(route('leader.announcements.update', { announcement: props.announcement.id }), {
        method: 'POST',
    ...
    <AppLayout :title="isDraft ? 'お知らせ編集（下書き）' : 'お知らせ編集'">
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('leader.announcements.show', { announcement: announcement.id })"
            ...
            <form @submit.prevent="submit" class="mx-auto max-w-2xl space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex gap-4">
                            <label class="flex cursor-pointer items-center gap-1.5">
                                <input type="radio" v-model="targetType" value="all" class="text-orange-600" /><span class="text-sm">全員</span>
                            </label>
                            <label class="flex cursor-pointer items-center gap-1.5">
                                <input type="radio" v-model="targetType" value="employees_only" class="text-orange-600" /><span class="text-sm">社員のみ</span>
                            </label>
                            <label class="flex cursor-pointer items-center gap-1.5">
                                <input type="radio" v-model="targetType" value="individual" class="text-orange-600" /><span class="text-sm">個別選択</span>
                            </label>
                        </div>
                ...
                <div>
                    ...
                    <textarea v-model="form.content" rows="8"
                        class="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-orange-400 focus:outline-none focus:ring-1 focus:ring-orange-400"></textarea>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="u in filteredModalUsers" :key="u.id"
                                    ...
                                    :class="selectedUserIds.includes(u.id) ? 'bg-orange-50' : ''"
                                    @click="toggleUser(u.id)">
                                    <td class="px-4 py-2 text-center">
                                        <input type="checkbox" :checked="selectedUserIds.includes(u.id)" @click.stop="toggleUser(u.id)" class="rounded text-orange-600" />
                                    </td>
