...
const departmentMembers = computed(() => {
    ...
    return users.value.filter(
        (u) =>
            ...
            !['superadmin', 'admin', 'leader'].includes((u.user_role || '').toString()),
    );
});
    ...
    <div class="mx-auto max-w-4xl rounded bg-white p-6 shadow">
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div class="flex justify-end">
                        <button type="submit" class="rounded bg-orange-600 px-4 py-2 text-white hover:bg-orange-700">更新</button>
                    </div>
                </form>
    </div>
</template>
