...
function sortWithChainMsg(items) {
    ...
    function appendWithChildren(item, depth) {
        ...
        visited.add(id);
        // グループをまたぐ続きジョブは既存の__chain_depthを尊重
...
const displayGroups = computed(() => {
    ...
    const messagesWithDepth = messages.map((m) => {
    ...
    for (const m of messagesWithDepth) {
...
// Coordinator 専用: ルートを直接指定（isCoordinator チェック不要）
function search() {
    const pjId = props.projectJob?.id;
    if (!pjId) {
        router.get(route('coordinator.jobbox'), { q: page.props.q_model, period: page.props.period_model }, { preserveState: false });
        return;
    }
    router.get(route('coordinator.project_jobs.jobbox.index', { projectJob: pjId }), { q: page.props.q_model, period: page.props.period_model }, { preserveState: false });
}
...
function getBackLink() {
    const pjId = props.projectJob?.id;
    try {
        if (!pjId) return route('coordinator.jobbox');
