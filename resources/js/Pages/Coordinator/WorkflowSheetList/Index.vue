...
async function openCreateModal() {
    ...
    if (createClients.value.length === 0) {
        ...
        try {
            const res = await fetch(route('coordinator.workflow_sheet_list.create_projects_json'), {
...
function goToCreateSheet() {
    ...
    sessionStorage.setItem('sbw_ws_create_return', 'workflow_sheet_list');
    router.visit(
        route('coordinator.project_jobs.show', { projectJob: createProjectId.value })
        + '?create_workflow_sheet=1',
    );
}
// ───────────────────────────────────────────────────────────────────────────
...
function search() {
    router.get(
        route('coordinator.workflow_sheet_list.index'),
        {
...
async function toggleFavorite(sheet) {
    ...
    try {
        const res = await axios.post(
            route('coordinator.workflow_sheet_list.favorite', { sheet: sheet.id }),
            {},
...
function openSheet(sheet) {
    router.get(route('coordinator.workflow_sheets.show', { sheet: sheet.id }));
}
</script>

<template>
    <AppLayout title="管理シート一覧">
        <template #header>
            <h2 class="text-base sm:text-xl font-semibold leading-tight text-gray-800">管理シート一覧</h2>
        </template>
                ...
                <table class="w-full text-sm">
                    <thead class="border-b bg-yellow-100 text-left text-xs text-yellow-900">
                        <tr>
                            ...
                            <th class="px-4 py-2 font-medium">管理シート</th>
            ...
            <div class="rounded bg-white p-4 shadow">
                <div class="flex flex-col gap-3 md:flex-row md:items-center">
                    <input
                        v-model="searchQuery"
                        @keyup.enter="search"
                        placeholder="案件名・管理シート名・クライアントで検索"
                        ...
                        <table class="w-full text-sm">
                            <thead class="border-b bg-white text-left text-xs text-gray-400">
                                <tr>
                                    ...
                                    <th class="px-4 py-1.5 font-medium">管理シート</th>
    ...
    <Teleport to="body">
        ...
        >
            <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
                <h2 class="mb-4 text-lg font-semibold text-gray-800">管理シートを新規作成</h2>
