...
function save() {
    ...
    const payload = {
        assignments: assignments.value.map((a) => ({
            ...
            // include lookup ids and amounts so server-side user store saves them
            work_item_type_id: a.work_item_type_id || null,
            size_id: a.size_id || null,
            stage_id: a.stage_id || null,
            status_id: a.status_id || null,
    ...
    // If all assignments are being created for the authenticated user, use the user-specific store endpoint
    try {
        const auth = effectiveAuthUser();
        const allForAuth = payload.assignments.every((x) => String(x.user_id) === String(auth ? auth.id : null));
        // payload prepared for submission (debug logging removed)

        // Determine projectJob id robustly: prefer provided prop, fall back to first assignment's project_job_id
        const computedProjectJobId = props.projectJob ? props.projectJob.id : payload.assignments[0] ? payload.assignments[0].project_job_id : '';

        if (allForAuth) {
            if (!computedProjectJobId) {
                // Prevent constructing a route with an empty project id which yields a double-slash URL and 404
            ...
            router.post(route('project_jobs.assignments.store_user', { projectJob: computedProjectJobId }), payload);
            return;
        }
    } catch (e) {
        // ignore and fall back to coordinator route
    }

    // default: coordinator store (preserves coordinator-specific side-effects)
    // Use the same computed fallback strategy for coordinator route
    const coordinatorProjectJobId = props.projectJob ? props.projectJob.id : payload.assignments[0] ? payload.assignments[0].project_job_id : '';
    ...
    router.post(route('coordinator.project_jobs.assignments.store', { projectJob: coordinatorProjectJobId }), payload);
}
</script>
