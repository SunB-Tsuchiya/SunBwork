...
function submit() {
    ...
    if (isEdit.value) {
        ...
        form.post(route('admin.meeting_definitions.store'), {
            onError: () => { errorMessage.value = '保存に失敗しました。'; },
        });
    }
}
            ...
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <div v-else>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        />
                        <button type="button" class="rounded border border-red-400 px-3 py-1.5 text-sm text-red-600 hover:bg-red-50" @click="addCustomDate(customDateInput)">
                            日付を追加
                        </button>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">カレンダーで選んだ日付を複数登録できます。</p>
                    <div class="mt-2 flex flex-wrap gap-1">
                        <span v-for="dateValue in form.custom_dates" :key="dateValue" class="inline-flex items-center gap-1 rounded bg-red-100 px-2 py-0.5 text-xs text-red-700">
                            {{ dateValue }}
