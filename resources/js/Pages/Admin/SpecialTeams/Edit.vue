...
const initMemberIds = (props.current_member_ids || []).map(String);
...
const submit = () => {
    ...
    form.put(route('admin.special_teams.update', { special_team: team.id }));
};
</script>

<template>
    <AppLayout title="特別チーム編集">
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-base sm:text-xl font-semibold leading-tight text-gray-800">特別チーム編集</h2>
            </div>
        </template>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100 bg-white">
                                <tr v-for="u in filteredUsers" :key="u.id"
                                    ...
                                    <td class="px-3 py-2 text-xs text-gray-500">{{ getCompanyName(u.company_id) }}</td>
                                    <td class="px-3 py-2 text-xs text-gray-500">{{ getDeptName(u.department_id) }}</td>
                                </tr>
