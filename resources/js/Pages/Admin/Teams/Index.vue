...
// 部署チームのみ表示（company・personal・unit は除外）
const departmentTeams = computed(() =>
    props.teams.filter((t) => t.team_type === 'department')
);
            ...
            <div v-else class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        <tr v-for="team in departmentTeams" :key="team.id" class="hover:bg-gray-50">
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-center">
                                <button
                                    @click="router.visit(route('admin.teams.show', { team: team.id }))"
                                    class="mr-1 rounded border border-gray-300 px-3 py-1 text-xs text-gray-700 hover:bg-gray-50"
                                >
                                    詳細
                                </button>
                                <button
                                    @click="router.visit(route('admin.teams.edit', { team: team.id }))"
                                    class="mr-1 rounded bg-indigo-600 px-3 py-1 text-xs font-bold text-white hover:bg-indigo-700"
                                >
                                    編集
                                </button>
                                <button
