...
const form = useForm({
    ...
    held_at:      props.minute.held_at ? String(props.minute.held_at).slice(0, 10) : '',
    attendee_ids: (props.minute.attendee_users ?? []).map(u => u.id),
});
...
function handleEditorReady(editor) {
    ...
    if (props.minute.content) {
        try {
            const delta = editor.clipboard.convert(props.minute.content);
            editor.setContents(delta);
        } catch {
            editor.root.innerHTML = props.minute.content;
        }
    }
...
function submit() {
    form.put(route('team-rooms.minutes.update', { team: props.team.id, minute: props.minute.id }));
}
</script>

<template>
    <AppLayout title="会議記録 編集">
        <template #header>
            <div class="flex items-center gap-3">
                ...
                >← 詳細に戻る</Link>
                <h2 class="text-xl font-semibold leading-tight text-gray-800">会議記録 編集 — {{ team.name }}</h2>
            </div>
        </template>
