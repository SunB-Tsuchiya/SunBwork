<template>
    <AppLayout :title="`JobBox - ${projectJob.name}`">
        ...
        <div class="mx-auto max-w-6xl rounded bg-white p-6 shadow">
                    ...
                    <tbody>
                        <tr v-for="m in messages.data" :key="m.id" class="hover:bg-gray-50">
                            ...
                            <td class="border px-4 py-2">
                                <Link :href="route('project_jobs.jobbox.show', { projectJob: projectJob.id, message: m.id })" class="rounded bg-blue-600 px-3 py-1 text-white">表示</Link>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="mt-4">
                <Link :href="route('project_jobs.show', { projectJob: projectJob.id })" class="rounded bg-gray-200 px-4 py-2">戻る</Link>
            </div>
        </div>
    </AppLayout>
</template>
...
function search() {
    router.get(route('project_jobs.jobbox.index', { projectJob: projectJob.id }), { q: page.props.q_model }, { preserveState: false });
}
