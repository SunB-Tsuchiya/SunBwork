        ...
        <div class="rounded bg-white p-6 shadow">
            ...
            <div class="mt-3 flex flex-wrap items-center gap-4">
                <div class="flex items-center gap-2">
                    ...
                    <select v-model="periodModel" @change="search" class="rounded border px-3 py-2 text-sm" style="width: 9.5em">
            ...
            <!-- グループ表示切替タブ -->
            <div class="mt-4 flex w-fit gap-1 rounded-lg border border-gray-200 bg-gray-50 p-1">
                <button
                    v-for="mode in viewModes"
                    :key="mode.key"
                    @click="viewMode = mode.key"
                    :class="viewMode === mode.key ? 'bg-white font-semibold text-blue-700 shadow-sm' : 'text-gray-600 hover:text-gray-900'"
            ...
            <div class="mt-4 overflow-x-auto">
                <div v-if="displayGroups.length === 0" class="py-8 text-center text-sm text-gray-400">表示するデータがありません。</div>
                ...
                <template v-for="group in displayGroups" :key="group.key">
                    ...
                    <table class="w-full table-fixed border" style="min-width: 640px">
                        ...
                        <tbody>
                            <tr v-for="job in group.items" :key="job.id" class="cursor-pointer hover:bg-blue-50" @click="goShow(job)" role="button">
            ...
            <!-- 件数 -->
            <div class="mt-4 text-sm text-gray-600">表示中 {{ totalDisplayCount }} 件</div>
        </div>
    </AppLayout>
</template>
...
function statusBadgeClass(job) {
    return job.completed ? 'bg-yellow-100 text-yellow-800' : 'bg-indigo-100 text-indigo-800';
}
...
const displayGroups = computed(() => {
    ...
    for (const job of jobs) {
        const key = viewMode.value === 'client' ? job.client?.name || '未設定' : getDateMonthKey(job);
