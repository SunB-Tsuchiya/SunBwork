<template>
    <AppLayout title="案件確認">
        <template #header>
            <h2 class="text-base sm:text-xl font-semibold leading-tight text-gray-800">案件確認</h2>
        </template>
        ...
        <div class="rounded bg-white px-4 py-6 sm:p-6 shadow">
            ...
            <div class="mt-3 flex flex-wrap items-center gap-4">
                ...
                <label class="flex cursor-pointer select-none items-center gap-2 text-sm text-gray-700">
                    ...
                    <table class="min-w-full border">
                        ...
                        <tbody>
                            <tr v-for="job in group.items" :key="job.id" class="cursor-pointer hover:bg-blue-50" @click="rowClick($event, job)">
                                ...
                                <td class="border px-3 py-2">
                                    <Link
                                        :href="route('user.project_jobs.show', { projectJob: job.id })"
...
function search() {
    try {
        router.get(route('user.project_jobs.index'), { q: qModel.value, period: periodModel.value }, { preserveState: false });
    } catch {
